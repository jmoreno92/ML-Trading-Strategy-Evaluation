...
class StrategyLearner(object):  		  	   		  	  		  		  		    	 		 		   		 		  
    ...
    def __init__(self, verbose=False, impact=0.0, commission=0.0):  		  	   		  	  		  		  		    	 		 		   		 		  
        ...
        # self.Qlearner = ql.QLearner(num_states=1000,num_actions=3,alpha=0.2,gamma=0.9,
        #                             rar=0.98,radr=0.999,dyna=0,verbose=False)  # initialize the learner
        self.Qlearner = ql.QLearner(num_states=30000,num_actions=3,alpha=0.2,gamma=0.9,
                                    rar=0.98,radr=0.999,dyna=0,verbose=False)  # initialize the learner
    ...
    def add_evidence(
        ...
        ### GET PRICES
        df_prices = self.getPrice(symbol, sd, ed, lookback=0)
        ...
        total_return = 0.1
        prev_total_return = -1000

        # percentage_difference = 0.05

        # while (count < countlim) & (abs(total_return/prev_total_return - 1) > percentage_difference):
        # while (count < countlim):     # Simple while statement that does not account for convergence
        while (count < countlim) & (total_return != prev_total_return):
            # Exit condition, when total return value does not change after iteration
            prev_total_return = total_return
            total_return = 0

            action = 1      # Initial action. # 2: LONG, 1: OUT, 0: SHORT
            position = 0
            prev_position = 0
            trade = 0
            cash = sv
            ...
            state = self.discretize(action, X, self.threshold)   # Obtain discretized state based on threshold array
            action = self.Qlearner.querysetstate(state)   # Set the state and get first action
            prev_action = action
            # 2: LONG, 1: OUT, 0: SHORT
            reward = 0  # reward is zero for 1st day


            # Return for first day
            sign = action - 1  # Calculate sign of action, 1: LONG, 0: OUT, -1: SHORT
            trade = sign * 1000
            position = sign * 1000

            prev_date = df_ind.index[0]
            ...
            total_return = cash + (position * prices.loc[first_date, symbol])
            ...
            for date in df_ind.index[1:]:
                prices = df_prices.loc[prev_date:date]      # Get prices of previous date and current date
                reward = total_return/prev_return - 1
                # reward = self.calcReward(prev_action, action,trade, prices)    # reward method considers impact
                prev_action = action
                action = self.Qlearner.query(state,reward)    # 2: LONG, 1: OUT, 0: SHORT
                sign = action - 1   # Calculate sign of action, 1: LONG, 0: OUT, -1: SHORT
                ...
                total_return  = cash + (position * prices.loc[date,symbol])
                # total_return += sign * 1000 * prices.loc[date, symbol]    # Original, do not use if possible
    ...
    def testPolicy(
        ...
        ### GET PRICES AND TRADES DATAFRAME BASED ON PRICES
        df_prices = self.getPrice(symbol, sd, ed, lookback = 0)
        ...
        for date in df_ind.index[1:]:
            ...
            prev_position = position
            position = sign * 1000
            trades.loc[date,symbol] = position - prev_position
    ...
    def discretize(self,action, s_ind, threshold):
        """
        Creates integer to represent 'state' based on Series of indicators, and threshold araray
        :param action: Action, 2: Buy, 1: Do nothing, 0: Sell
        :type position: int
    ...
    def calcReward(self,prev_action, action,trade,prices):
        # actions    2: LONG, 1: OUT, 0: SHORT
        if prev_action == action or trade == 0:
            sign = action - 1
            reward = sign * (prices.iloc[1,0]/prices.iloc[0,0] - 1)
            return reward
            ...
            # reward = np.sign(trade) * ( ((prices.iloc[1, 0] - prices.iloc[0, 0]) * 1000) - np.sign(trade) * fee ) / (prices.iloc[0, 0] * 1000)
            reward = (sign*((prices.iloc[1, 0] - prices.iloc[0, 0]) * 1000) - fee) / (prices.iloc[0, 0] * 1000)
            return reward
